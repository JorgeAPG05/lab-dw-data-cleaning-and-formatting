...
def clean_dataframe(df: pd.DataFrame, options: dict={"column_name": "state"}) -> pd.DataFrame:
    ...
    df2= clean_state_column(df2)
    df2= clean_education_column(df2)
    df2= clean_customerlifetimevalue_column(df2)
    df2= clean_vehicle_class_column(df2)
    df2= correct_open_complaints(df2)
    df2= fill_nan_dataframe(df2)
